...
///
/// 说明：
/// - 该文件主要是静态映射表（干支、节气、方位、神煞等）。
/// - 业务方法集中在底部若干 `get...` 工具函数。
enum LunarUtil {
    ...
    static let XUN: [String] = [
    "甲子", "甲戌", "甲申", "甲午", "甲辰", "甲寅"
]

    static let XUN_KONG: [String] = [
    "戌亥", "申酉", "午未", "辰巳", "寅卯", "子丑"
]

    static let LIU_YAO: [String] = [
    "先胜", "友引", "先负", "佛灭", "大安", "赤口"
]

    static let HOU: [String] = [
    "初候", "二候", "三候"
]

    static let WU_HOU: [String] = [
    "蚯蚓结", "麋角解", "水泉动", "雁北乡", "鹊始巢", "雉始雊", "鸡始乳", "征鸟厉疾",
    "水泽腹坚", "东风解冻", "蛰虫始振", "鱼陟负冰", "獭祭鱼", "候雁北", "草木萌动", "桃始华",
    "仓庚鸣", "鹰化为鸠", "玄鸟至", "雷乃发声", "始电", "桐始华", "田鼠化为鴽", "虹始见",
    "萍始生", "鸣鸠拂其羽", "戴胜降于桑", "蝼蝈鸣", "蚯蚓出", "王瓜生", "苦菜秀", "靡草死",
    "麦秋至", "螳螂生", "鵙始鸣", "反舌无声", "鹿角解", "蜩始鸣", "半夏生", "温风至",
    "蟋蟀居壁", "鹰始挚", "腐草为萤", "土润溽暑", "大雨行时", "凉风至", "白露降", "寒蝉鸣",
    "鹰乃祭鸟", "天地始肃", "禾乃登", "鸿雁来", "玄鸟归", "群鸟养羞", "雷始收声", "蛰虫坯户",
    "水始涸", "鸿雁来宾", "雀入大水为蛤", "菊有黄花", "豺乃祭兽", "草木黄落", "蛰虫咸俯", "水始冰",
    "地始冻", "雉入大水为蜃", "虹藏不见", "天气上升地气下降", "闭塞而成冬", "鹖鴠不鸣", "虎始交", "荔挺出"
]

    static let GAN: [String] = [
    "", "甲", "乙", "丙", "丁", "戊", "己", "庚",
    "辛", "壬", "癸"
]

    static let POSITION_XI: [String] = [
    "", "艮", "乾", "坤", "离", "巽", "艮", "乾",
    "坤", "离", "巽"
]

    static let POSITION_YANG_GUI: [String] = [
    "", "坤", "坤", "兑", "乾", "艮", "坎", "离",
    "艮", "震", "巽"
]

    static let POSITION_YIN_GUI: [String] = [
    "", "艮", "坎", "乾", "兑", "坤", "坤", "艮",
    "离", "巽", "震"
]

    static let POSITION_FU: [String] = [
    "", "巽", "巽", "震", "震", "坎", "离", "坤",
    "坤", "乾", "兑"
]

    static let POSITION_FU_2: [String] = [
    "", "坎", "坤", "乾", "巽", "艮", "坎", "坤",
    "乾", "巽", "艮"
]

    static let POSITION_CAI: [String] = [
    "", "艮", "艮", "坤", "坤", "坎", "坎", "震",
    "震", "离", "离"
]

    static let POSITION_DESC: [String: String] = [
    "坎": "正北",
    "艮": "东北",
    "震": "正东",
    "巽": "东南",
    ...
    static let POSITION_TAI_SUI_YEAR: [String] = [
    "坎", "艮", "艮", "震", "巽", "巽", "离", "坤",
    "坤", "兑", "坎", "坎"
]

    static let POSITION_GAN: [String] = [
    "震", "震", "离", "离", "中", "中", "兑", "兑",
    "坎", "坎"
]

    static let POSITION_ZHI: [String] = [
    "坎", "中", "震", "震", "中", "离", "离", "中",
    "兑", "兑", "中", "坎"
]

    static let POSITION_TAI_DAY: [String] = [
    "占门碓 外东南", "碓磨厕 外东南", "厨灶炉 外正南", "仓库门 外正南", "房床栖 外正南", "占门床 外正南", "占碓磨 外正南", "厨灶厕 外西南",
    "仓库炉 外西南", "房床门 外西南", "占门栖 外西南", "碓磨床 外西南", "厨灶碓 外西南", "仓库厕 外正西", "房床炉 外正西", "占大门 外正西",
    "碓磨栖 外正西", "厨灶床 外正西", "仓库碓 外西北", "房床厕 外西北", "占门炉 外西北", "碓磨门 外西北", "厨灶栖 外西北", "仓库床 外西北",
    "房床碓 外正北", "占门厕 外正北", "碓磨炉 外正北", "厨灶门 外正北", "仓库栖 外正北", "占房床 房内北", "占门碓 房内北", "碓磨厕 房内北",
    "厨灶炉 房内北", "仓库门 房内北", "房床栖 房内中", "占门床 房内中", "占碓磨 房内南", "厨灶厕 房内南", "仓库炉 房内南", "房床门 房内西",
    "占门栖 房内东", "碓磨床 房内东", "厨灶碓 房内东", "仓库厕 房内东", "房床炉 房内中", "占大门 外东北", "碓磨栖 外东北", "厨灶床 外东北",
    "仓库碓 外东北", "房床厕 外东北", "占门炉 外东北", "碓磨门 外正东", "厨灶栖 外正东", "仓库床 外正东", "房床碓 外正东", "占门厕 外正东",
    "碓磨炉 外东南", "厨灶门 外东南", "仓库栖 外东南", "占房床 外东南"
]

    static let POSITION_TAI_MONTH: [String] = [
    "占房床", "占户窗", "占门堂", "占厨灶", "占房床", "占床仓", "占碓磨", "占厕户",
    "占门房", "占房床", "占灶炉", "占房床"
]
    ...
    static let TIAN_SHEN: [String] = [
    "", "青龙", "明堂", "天刑", "朱雀", "金匮", "天德", "白虎",
    "玉堂", "天牢", "玄武", "司命", "勾陈"
]

    static let ZHI_TIAN_SHEN_OFFSET: [String: Int] = [
    "子": 4,
    "丑": 2,
    "寅": 0,
    "卯": 10,
    ...
    static let TIAN_SHEN_TYPE: [String: String] = [
    "青龙": "黄道",
    "明堂": "黄道",
    "金匮": "黄道",
    "天德": "黄道",
    ...
    static let TIAN_SHEN_TYPE_LUCK: [String: String] = [
    "黄道": "吉",
    "黑道": "凶"
]

    static let PENGZU_GAN: [String] = [
    "", "甲不开仓财物耗散", "乙不栽植千株不长", "丙不修灶必见灾殃", "丁不剃头头必生疮", "戊不受田田主不祥", "己不破券二比并亡", "庚不经络织机虚张",
    "辛不合酱主人不尝", "壬不泱水更难提防", "癸不词讼理弱敌强"
]

    static let PENGZU_ZHI: [String] = [
    "", "子不问卜自惹祸殃", "丑不冠带主不还乡", "寅不祭祀神鬼不尝", "卯不穿井水泉不香", "辰不哭泣必主重丧", "巳不远行财物伏藏", "午不苫盖屋主更张",
    "未不服药毒气入肠", "申不安床鬼祟入房", "酉不会客醉坐颠狂", "戌不吃犬作怪上床", "亥不嫁娶不利新郎"
]
    ...
    static let SEASON: [String] = [
    "", "孟春", "仲春", "季春", "孟夏", "仲夏", "季夏", "孟秋",
    "仲秋", "季秋", "孟冬", "仲冬", "季冬"
]

    static let SHENGXIAO: [String] = [
    "", "鼠", "牛", "虎", "兔", "龙", "蛇", "马",
    "羊", "猴", "鸡", "狗", "猪"
]
    ...
    static let YUE_XIANG: [String] = [
    "", "朔", "既朔", "蛾眉新", "蛾眉新", "蛾眉", "夕", "上弦",
    "上弦", "九夜", "宵", "宵", "宵", "渐盈凸", "小望", "望",
    "既望", "立待", "居待", "寝待", "更待", "渐亏凸", "下弦", "下弦",
    "有明", "有明", "蛾眉残", "蛾眉残", "残", "晓", "晦"
]

    static let XIU: [String: String] = [
    "申1": "毕",
    "申2": "翼",
    "申3": "箕",
    "申4": "奎",
    ...
    static let XIU_LUCK: [String: String] = [
    "角": "吉",
    "亢": "凶",
    "氐": "凶",
    "房": "吉",
    ...
    static let XIU_SONG: [String: String] = [
    ...
    static let ZHENG: [String: String] = [
    "角": "木",
    "井": "木",
    "奎": "木",
    "斗": "木",
    ...
    static let ANIMAL: [String: String] = [
    "角": "蛟",
    "斗": "獬",
    "奎": "狼",
    "井": "犴",
    ...
    static let CHONG: [String] = [
    "午", "未", "申", "酉", "戌", "亥", "子", "丑",
    "寅", "卯", "辰", "巳"
]

    static let CHONG_GAN: [String] = [
    "戊", "己", "庚", "辛", "壬", "癸", "甲", "乙",
    "丙", "丁"
]

    static let CHONG_GAN_TIE: [String] = [
    "己", "戊", "辛", "庚", "癸", "壬", "乙", "甲",
    "丁", "丙"
]

    static let CHONG_GAN_4: [String] = [
    "庚", "辛", "壬", "癸", "", "", "甲", "乙",
    "丙", "丁"
]
    ...
    static let NAYIN: [String: String] = [
    "甲子": "海中金",
    "甲午": "沙中金",
    "丙寅": "炉中火",
    "丙申": "山下火",
    ...
    static let HE_GAN_5: [String] = [
    "己", "庚", "辛", "壬", "癸", "甲", "乙", "丙",
    "丁", "戊"
]

    static let HE_ZHI_6: [String] = [
    "丑", "子", "亥", "戌", "酉", "申", "未", "午",
    "巳", "辰", "卯", "寅"
]

    static let LU: [String: String] = [
    "甲": "寅",
    "乙": "卯",
    "丙": "巳",
    "丁": "午",
    ...
    static let ZHI_HIDE_GAN: [String: [String]] = [
    "子": [
        "癸"
],
    "丑": [
    ...
    static let SHI_SHEN: [String: String] = [
    "甲甲": "比肩",
    "甲乙": "劫财",
    "甲丙": "食神",
    "甲丁": "伤官",
    ...
    static func convertTime(_ hm: String) -> String {
        return ZHI[getTimeZhiIndex(hm) + 1]
    }

    static func getJiaZiIndex(_ ganZhi: String) -> Int {
        return index(ganZhi, JIA_ZI, 0)
    }

    static func hex(_ n: Int) -> String {
        let hex = String(format: "%02X", n)
        return hex
    }

    static func getDayYi(_ monthGanZhi: String, _ dayGanZhi: String) -> [String] {
        var list: [String] = []
        let day = hex(getJiaZiIndex(dayGanZhi))
        let month = hex(getJiaZiIndex(monthGanZhi))
        var right = DAY_YI_JI
        var index = right.range(of: day + "=")
        while let r = index {
            right = String(right[r.upperBound...])
            var left = right
            if let eq = left.range(of: "=") {
                left = String(left[..<left.index(eq.lowerBound, offsetBy: -2)])
            }
            let months = String(left[..<left.firstIndex(of: ":")!])
            var matched = false
            var i = 0
            while i < months.count {
                let start = months.index(months.startIndex, offsetBy: i)
                let end = months.index(start, offsetBy: 2)
                if String(months[start..<end]) == month { matched = true; break }
                i += 2
            }
            if matched {
                var ys = String(left[left.index(after: left.firstIndex(of: ":")!)...])
                ys = String(ys[..<ys.firstIndex(of: ",")!])
                var j = 0
                while j < ys.count {
                    let s = ys.index(ys.startIndex, offsetBy: j)
                    let e = ys.index(s, offsetBy: 2)
                    let v = Int(ys[s..<e], radix: 16) ?? 0
                    list.append(YI_JI[v])
                    j += 2
                }
                break
        ...
        if list.isEmpty { list.append(SHEN_SHA[0]) }
        return list
    }

    static func getDayJi(_ monthGanZhi: String, _ dayGanZhi: String) -> [String] {
        var list: [String] = []
        let day = hex(getJiaZiIndex(dayGanZhi))
        let month = hex(getJiaZiIndex(monthGanZhi))
        var right = DAY_YI_JI
        ...
        while let r = index {
            ...
            if matched {
                let js = String(left[left.index(after: left.firstIndex(of: ",")!)...])
                var j = 0
                while j < js.count {
                    let s = js.index(js.startIndex, offsetBy: j)
                    let e = js.index(s, offsetBy: 2)
                    let v = Int(js[s..<e], radix: 16) ?? 0
                    list.append(YI_JI[v])
                    j += 2
                }
                break
    ...
    static func getDayJiShen(_ monthZhiIndex: Int, _ dayGanZhi: String) -> [String] {
        var list: [String] = []
        var m = monthZhiIndex - 2
        if m < 0 { m += 12 }
        var index = String(getJiaZiIndex(dayGanZhi), radix: 16).uppercased()
        if index.count < 2 { index = "0" + index }
        if let matcher = DAY_SHEN_SHA[m].range(of: ";" + index) {
            let right = String(DAY_SHEN_SHA[m][matcher.upperBound...])
            if let semi = right.firstIndex(of: ";") {
                let data = String(right[..<semi])
                var i = 0
                while i < data.count {
                    let s = data.index(data.startIndex, offsetBy: i)
                    let e = data.index(s, offsetBy: 2)
                    let n = Int(data[s..<e], radix: 16) ?? 0
                    if n < 60 { list.append(SHEN_SHA[n + 1]) }
                    i += 2
                }
            }
        }
    ...
    static func getDayXiongSha(_ monthZhiIndex: Int, _ dayGanZhi: String) -> [String] {
        var list: [String] = []
        var m = monthZhiIndex - 2
        if m < 0 { m += 12 }
        var index = String(getJiaZiIndex(dayGanZhi), radix: 16).uppercased()
    ...
    static func getTimeYi(_ dayGanZhi: String, _ timeGanZhi: String) -> [String] {
        var list: [String] = []
        let day = hex(getJiaZiIndex(dayGanZhi))
        let time = hex(getJiaZiIndex(timeGanZhi))
        if let range = TIME_YI_JI.range(of: day + time + "=") {
            var left = String(TIME_YI_JI[range.upperBound...])
            if let eq = left.range(of: "=") {
                left = String(left[..<left.index(eq.lowerBound, offsetBy: -4)])
            }
            let ys = String(left[..<left.firstIndex(of: ",")!])
            var i = 0
            while i < ys.count {
                let s = ys.index(ys.startIndex, offsetBy: i)
                let e = ys.index(s, offsetBy: 2)
                let v = Int(ys[s..<e], radix: 16) ?? 0
                list.append(YI_JI[v])
                i += 2
    ...
    static func getTimeJi(_ dayGanZhi: String, _ timeGanZhi: String) -> [String] {
        var list: [String] = []
        let day = hex(getJiaZiIndex(dayGanZhi))
        let time = hex(getJiaZiIndex(timeGanZhi))
        if let range = TIME_YI_JI.range(of: day + time + "=") {
            ...
            var i = 0
            while i < js.count {
                let s = js.index(js.startIndex, offsetBy: i)
                let e = js.index(s, offsetBy: 2)
                let v = Int(js[s..<e], radix: 16) ?? 0
                list.append(YI_JI[v])
                i += 2
    ...
    static func getXunIndex(_ ganZhi: String) -> Int {
        let gan = find(ganZhi, GAN)?.index ?? 0
        let zhi = find(ganZhi, ZHI)?.index ?? 0
        var diff = gan - zhi
        if diff < 0 { diff += 12 }
        return diff / 2
    }

    static func getXun(_ ganZhi: String) -> String {
        return XUN[getXunIndex(ganZhi)]
    }

    static func getXunKong(_ ganZhi: String) -> String {
        return XUN_KONG[getXunIndex(ganZhi)]
    }

    static func index(_ name: String, _ names: [String], _ offset: Int) -> Int {
        for (i, v) in names.enumerated() where v == name { return i + offset }
        return -1
    }

    static func find(_ s: String, _ arr: [String]) -> (index: Int, value: String)? {
        for (i, v) in arr.enumerated() {
            if v.isEmpty { continue }
            if s.contains(v) { return (i, v) }
        }
        return nil
    }
}
